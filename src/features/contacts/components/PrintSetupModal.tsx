// PrintSetupModal.tsx
// Lets the user pick which columns to include before opening the print-preview tab.
// Persists choices to localStorage per role.
...
// Columns that are structural/interactive and make no sense on paper.
const SYSTEM_COLS = new Set(["select", "sequence", "changeStatus"]);

const PRINT_STORAGE_PREFIX = "fs-print-cols-";
...
function loadPrintCols(roleKey) {
  try {
    const raw = localStorage.getItem(PRINT_STORAGE_PREFIX + roleKey);
...
function savePrintCols(roleKey, cols) {
  try {
    localStorage.setItem(PRINT_STORAGE_PREFIX + roleKey, JSON.stringify(cols));
  } catch { /* ignore */ }
}
 ...
 * @param {function} props.onConfirm      - Called with { columns: string[], printSelected: boolean }
...
export default function PrintSetupModal({
  ...
  // Printable = everything except structural system columns
  const printable = (allColumnIds ?? []).filter(id => !SYSTEM_COLS.has(id));

  const resolveInitial = useCallback(() => {
    const saved     = loadPrintCols(roleKey);
    const available = new Set(printable);
    if (saved?.length) {
      return new Set(saved.filter(id => available.has(id)));
    }
    return new Set(printable); // default: all on
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [roleKey, printable.join(",")]);
  ...
  useEffect(() => {
    if (open) {
      ...
      setPrintSelected(selectedCount > 0 && roleKey === "admin");
    }
  ...
  const selectAll   = () => setSelected(new Set(printable));
  const deselectAll = () => setSelected(new Set());

  const handleConfirm = () => {
    const ordered = printable.filter(id => selected.has(id));
    savePrintCols(roleKey, ordered);
    onConfirm({ columns: ordered, printSelected });
  };

  // ── Derived ─────────────────────────────────────────────────────────────────

  const allChecked  = printable.every(id => selected.has(id));
    ...
    >
      ...
      {selectedCount > 0 && roleKey === "admin" && (
        <div style={{
          ...
          <label style={{
            ...
            cursor:     "pointer",
            fontSize:   13,
            fontWeight: !printSelected ? 600 : 400,
            color:      !printSelected ? "var(--fs-text)" : "var(--fs-text-dim)",
          }}>
            <input
              type="radio"
              name="printScope"
              checked={!printSelected}
              onChange={() => setPrintSelected(false)}
          ...
          <label style={{
            ...
            <input
              ...
              name="printScope"
              checked={printSelected}
              onChange={() => setPrintSelected(true)}
              style={{ accentColor: "var(--fs-accent)" }}
            />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <span style={{
          ...
          {selected.size}&thinsp;/&thinsp;{printable.length}
        </span>
      </div>
      ...
      {noneChecked && (
        <div style={{
          ...
          Select at least one column to enable printing.
        </div>
      )}
